...
class TestOperatorLogic(unittest.TestCase):
    ...
    def test_and_operator_basic(self):
        ...
        success, _, _, _, _ = self.sim.check_success(["Starter", "Extender", "Other"], [rule])
        self.assertTrue(success)
        
        # Should fail with only Starter
        success, _, _, _, _ = self.sim.check_success(["Starter", "Other", "Other"], [rule])
        self.assertFalse(success)
        
        # Should fail with only Extender
        success, _, _, _, _ = self.sim.check_success(["Extender", "Other", "Other"], [rule])
        self.assertFalse(success)
    ...
    def test_or_operator_basic(self):
        ...
        success, _, _, _, _ = self.sim.check_success(["Other", "Other", "Other"], [rule])
        self.assertFalse(success)
    ...
    def test_mixed_and_or_operators(self):
        ...
        success, _, _, _, _ = self.sim.check_success(["Starter", "Handtrap", "Other"], [rule])
        self.assertTrue(success)
        
        # Should succeed with all three
        success, _, _, _, _ = self.sim.check_success(["Starter", "Extender", "Handtrap"], [rule])
        self.assertTrue(success)
        ...
        success, _, _, _, _ = self.sim.check_success(["Extender", "Handtrap", "Other"], [rule])
        self.assertFalse(success)
    ...
    def test_complex_or_chain(self):
        ...
        success, _, _, _, _ = self.sim.check_success(["Extender", "Other", "Other"], [rule])
        self.assertTrue(success)
        success, _, _, _, _ = self.sim.check_success(["Handtrap", "Other", "Other"], [rule])
        self.assertTrue(success)
